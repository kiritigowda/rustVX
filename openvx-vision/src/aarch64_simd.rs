...
pub mod neon {
    ...
    #[target_feature(enable = "neon")]
    pub unsafe fn weighted_avg_neon(src1: *const u8, src2: *const u8, dst: *mut u8, len: usize, alpha: u8) {
        let beta = 255 - alpha;
        ...
        for y in 0..height {
            ...
            *dst.add(row_offset) = *src.add(row_offset);
            ...
            while x + 15 < width - 1 {
                ...
                let sum_low = vaddq_u16(
                    vaddq_u16(prev_low, next_low),
                    vshlq_n_u16(curr_low, 1)
                );
                let sum_high = vaddq_u16(
                    vaddq_u16(prev_high, next_high),
                    vshlq_n_u16(curr_high, 1)
                );
