...
pub fn rgb_to_gray_simd(src: &Image, dst: &Image) -> VxResult<()> {
    ...
    unsafe {
        ...
        crate::simd_utils::scalar::rgb_to_gray_scalar(&src_data, &mut dst_data, num_pixels);
    }
...
pub fn gray_to_rgb_simd(src: &Image, dst: &Image) -> VxResult<()> {
    ...
    unsafe {
        ...
        for i in 0..chunks {
            ...
            _mm_storeu_si128(
                dst_data.as_mut_ptr().add(i * 48 + 16) as *mut __m128i,
                rgb0_hi,
            );
            _mm_storeu_si128(
                dst_data.as_mut_ptr().add(i * 48 + 32) as *mut __m128i,
                rgb1_lo,
            );
        }
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            if y % 2 == 0 && x % 2 == 0 {
                ...
                let r2 = if x + 1 < width {
                    src_data[(y * width + x + 1) * 3] as i32
                } else {
                    r
                };
                let r3 = if y + 1 < height {
                    src_data[((y + 1) * width + x) * 3] as i32
                } else {
                    r
                };
                ...
                let g2 = if x + 1 < width {
                    src_data[(y * width + x + 1) * 3 + 1] as i32
                } else {
                    g
                };
                let g3 = if y + 1 < height {
                    src_data[((y + 1) * width + x) * 3 + 1] as i32
                } else {
                    g
                };
                ...
                let b2 = if x + 1 < width {
                    src_data[(y * width + x + 1) * 3 + 2] as i32
                } else {
                    b
                };
                let b3 = if y + 1 < height {
                    src_data[((y + 1) * width + x) * 3 + 2] as i32
                } else {
                    b
                };
