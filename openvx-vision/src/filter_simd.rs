...
pub fn gaussian3x3_simd(src: &Image, dst: &Image) -> VxResult<()> {
    ...
    #[cfg(not(any(target_arch = "x86_64", target_arch = "aarch64")))]
    {
        ...
        crate::simd_utils::scalar::gaussian_h3_scalar(
            &src_data,
            &mut temp_buffer,
            width,
            height
        );
        crate::simd_utils::scalar::gaussian_v3_scalar(
            &temp_buffer,
            &mut dst_data,
            width,
            height
        ...
        for x in 0..width {
            ...
            for k in 0..5 {
                ...
                if py >= 0 && py < height as isize {
                    sum += temp_buffer[py as usize * width + x] as i32 * crate::utils::GAUSSIAN_5X5[k];
                    weight += crate::utils::GAUSSIAN_5X5[k];
                }
            }
...
pub fn box3x3_simd(src: &Image, dst: &Image) -> VxResult<()> {
    ...
    for y in 0..height {
        ...
        for x in 1..width - 1 {
            ...
            // Update window
            if x + 2 < width {
                window_sum = window_sum + src_data[y * width + x + 2] as u32 - src_data[y * width + x - 1] as u32;
            }
        }
        
        // Handle edges
        temp_buffer[y * width] = ((src_data[y * width] as u16 + 
                                   src_data[y * width + 1] as u16) / 2) as u8;
    ...
    for x in 0..width {
        // Initialize sliding window sum
        let mut window_sum = (temp_buffer[x] as u32 + 
                            temp_buffer[width + x] as u32) * 2 + 
        ...
        for y in 1..height - 1 {
            ...
            // Update window
            if y + 2 < height {
                window_sum = window_sum + temp_buffer[(y + 2) * width + x] as u32 - temp_buffer[(y - 1) * width + x] as u32;
            }
        }
        ...
        dst_data[(height - 1) * width + x] = ((temp_buffer[(height - 2) * width + x] as u16 + 
                                               temp_buffer[(height - 1) * width + x] as u16) / 2) as u8;
    }
