...
fn test_add_basic() {
    ...
    for y in 0..height {
        for x in 0..width {
            assert_eq!(dst_data[y * width + x], 150, "Addition failed at ({}, {})", x, y);
        }
    }
}
...
fn test_add_saturation() {
    ...
    for y in 0..height {
        for x in 0..width {
            assert_eq!(dst_data[y * width + x], 255, "Saturation failed at ({}, {})", x, y);
        }
    }
}
...
fn test_subtract_basic() {
    ...
    for y in 0..height {
        for x in 0..width {
            assert_eq!(dst_data[y * width + x], 100, "Subtraction failed at ({}, {})", x, y);
        }
    }
}
...
fn test_subtract_floor() {
    ...
    for y in 0..height {
        for x in 0..width {
            assert_eq!(dst_data[y * width + x], 0, "Flooring failed at ({}, {})", x, y);
        }
    }
}
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            assert!((actual as i16 - expected as i16).abs() <= 2, 
                "Multiplication failed at ({}, {}): expected {}, got {}", x, y, expected, actual);
        }
    }
}
...
fn test_weighted_average() {
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            assert!(diff <= 1, "Weighted average failed at ({}, {}): expected {}, got {}", 
                x, y, expected, actual);
        }
    }
}
...
fn test_weighted_full_alpha() {
    ...
    for y in 0..height {
        for x in 0..width {
            let actual = dst_data[y * width + x];
            assert!(actual > 195, "Alpha=255 should give nearly full src1 weight: got {}", actual);
        }
    }
}
