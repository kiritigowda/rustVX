...
fn test_magnitude_computation() {
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            assert!(diff <= 2, "Magnitude mismatch at ({}, {}): expected {}, got {}", x, y, expected, actual);
        }
    }
}
...
fn test_phase_computation() {
    ...
    for &v in phase_data.iter() {
        ...
        assert!(diff <= 10, "Phase computation mismatch: expected around {}, got {}", expected, v);
    }
}
...
fn test_gradient_symmetry() {
    ...
    for y in 1..4 {
        for x in 1..4 {
            ...
            assert!(gx.abs_diff(128) <= 2, "Uniform image should have zero gradient X");
            assert!(gy.abs_diff(128) <= 2, "Uniform image should have zero gradient Y");
        }
    }
}
