...
fn test_harris_corners_finds_corners() {
    ...
    let input = Image::new(9, 9, ImageFormat::Gray);
...
fn test_harris_corners_no_corners_on_uniform() {
    ...
    assert!(
        corners.is_empty(),
        "Harris should find no corners on uniform image"
    );
}
...
fn test_fast9_finds_corners() {
    ...
    let _corners = fast9(&input, 30).unwrap();
...
fn test_fast9_on_uniform() {
    ...
    assert!(
        corners.is_empty(),
        "FAST9 should find no corners on uniform image"
    );
}
...
fn test_fast12_finds_corners() {
    ...
    let input = Image::new(15, 15, ImageFormat::Gray);
    ...
    let _corners = fast9(&input, 30).unwrap(); // Use FAST-9 first to verify it would find something
...
fn test_non_max_suppression() {
    ...
    assert!(
        corners.len() <= 2,
        "Non-max suppression should reduce nearby corners"
    );
}
...
fn test_corner_sorting() {
    ...
    assert_eq!(
        corners[0].strength, 50.0,
        "Corners should be sorted by strength"
    );
