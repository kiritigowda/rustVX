...
fn bench_gaussian3x3(c: &mut Criterion) {
    ...
    for (width, height) in &sizes {
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    openvx_vision::filter::gaussian3x3(black_box(&src), black_box(&mut dst))
                        .unwrap()
                })
            },
        );
...
fn bench_box3x3(c: &mut Criterion) {
    ...
    for (width, height) in &sizes {
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    openvx_vision::filter::box3x3(black_box(&src), black_box(&mut dst)).unwrap()
                })
            },
        );
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    openvx_vision::filter_simd::box3x3_simd(black_box(&src), black_box(&mut dst))
                        .unwrap()
                })
            },
        );
...
fn bench_rgb_to_gray(c: &mut Criterion) {
    ...
    for (width, height) in &sizes {
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    openvx_vision::color::rgb_to_gray(black_box(&src), black_box(&mut dst)).unwrap()
                })
            },
        );
...
fn bench_sobel3x3(c: &mut Criterion) {
    ...
    for (width, height) in &sizes {
        ...
        group.bench_with_input(
            BenchmarkId::new("scalar", format!("{}x{}", width, height)),
            &(*width, *height),
            |b, _| b.iter(|| openvx_vision::gradient::sobel3x3_full(black_box(&src)).unwrap()),
        );
