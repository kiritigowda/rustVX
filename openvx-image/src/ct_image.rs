...
use openvx_core::c_api::{
    vx_context, vx_df_image, vx_enum, vx_image, vx_imagepatch_addressing_t, vx_map_id,
    vx_rectangle_t, vx_uint32, VX_IMAGE_FORMAT, VX_IMAGE_HEIGHT, VX_IMAGE_WIDTH,
    VX_MEMORY_TYPE_HOST, VX_READ_ONLY, VX_SUCCESS, VX_WRITE_ONLY,
};
...
pub extern "C" fn ct_image_data_replicate_8u(image: CT_Image, x: i32, y: i32) -> u8 {
    ...
    unsafe {
        ...
        *img.data
            .y
            .add((clamped_y * img.stride + clamped_x) as usize)
    }
}
...
#[no_mangle]
pub extern "C" fn ct_image_to_vx_image(ctimg: CT_Image, context: vx_context) -> vx_image {
    use crate::c_api::*;
        ...
        if is_planar {
            ...
            for plane_idx in 0..num_planes {
                ...
                let (plane_width, plane_height) =
                    VxCImage::plane_dimensions(width, height, vx_format, plane_idx);
                ...
                let status = vxMapImagePatch(
                    vximg,
                    &rect,
                    plane_idx as vx_uint32,
                    &mut map_id,
                    &mut addr,
                    &mut ptr,
                    VX_WRITE_ONLY,
                ...
                if !ptr.is_null() && !img.data_begin_.is_null() {
                    ...
                    let src_row_stride = if (vx_format == VX_DF_IMAGE_NV12
                    ...
                    for y in 0..plane_height as usize {
                        ...
                        if row_size > 0
                            && plane_offset + y * src_row_stride + row_size <= img.data_size_
                        {
...
pub extern "C" fn ct_image_from_vx_image(vximg: vx_image) -> CT_Image {
    ...
    unsafe {
        ...
        if vxQueryImage(
            vximg,
            VX_IMAGE_WIDTH,
            &mut width as *mut _ as *mut std::ffi::c_void,
        ...
        if vxQueryImage(
            vximg,
            VX_IMAGE_HEIGHT,
            &mut height as *mut _ as *mut std::ffi::c_void,
            std::mem::size_of::<vx_uint32>(),
        ) != VX_SUCCESS
        {
            return std::ptr::null_mut();
        ...
        if is_planar {
            ...
            for plane_idx in 0..num_planes {
                ...
                // Get plane dimensions
                let (plane_width, plane_height) =
                    VxCImage::plane_dimensions(width, height, format, plane_idx);
                // Per OpenVX spec, rect is in full-image coordinates
                let rect = vx_rectangle_t {
                    start_x: 0,
                    start_y: 0,
                ...
                if !ptr.is_null() && !img.data_begin_.is_null() {
                    ...
                    // CT_Image stores NV12/NV21 UV with row stride = width (full image width)
                    let dst_row_stride = if (format == VX_DF_IMAGE_NV12
                    ...
                    for y in 0..plane_height as usize {
                        ...
                        if row_size > 0
                            && plane_offset + y * dst_row_stride + row_size <= img.data_size_
                        {
                            std::ptr::copy_nonoverlapping(src_row, dst_row, row_size);
                        }
                    }
...
#[no_mangle]
pub extern "C" fn ct_image_copy(ctimg: CT_Image, vximg: vx_image, dir: CT_ImageCopyDirection) {
    ...
    unsafe {
        ...
        if !img.data_begin_.is_null() && !cl.data_begin_.is_null() && img.data_size_ > 0 {
            std::ptr::copy_nonoverlapping(
                img.data_begin_,
                cl.data_begin_,
                img.data_size_.min(cl.data_size_),
            );
        }
...
#[no_mangle]
pub extern "C" fn ct_fill_ct_image_random(image: CT_Image, seed: *mut u64, a: i32, b: i32) {
    if image.is_null() {
        return;
    }
    ...
    unsafe {
        let img = &mut *image;
        let mut rng = if seed.is_null() { 123456789u64 } else { *seed };
...
pub extern "C" fn ct_dump_image_info(image: CT_Image) {
    ...
    unsafe {
        let img = &*image;
        debug!(
            "CT_Image: {}x{} stride={} format=0x{:08X}",
...
#[no_mangle]
pub extern "C" fn ct_read_image(_file_name: *const i8, _dcn: i32) -> CT_Image {
...
#[no_mangle]
pub extern "C" fn ct_write_image(_file_name: *const i8, _image: CT_Image) {
    // Stub implementation
}
    ...
    unsafe {
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                if image.format as u32 == 0x20200100 {
                    // VX_DF_IMAGE_S32
