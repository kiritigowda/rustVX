// Simple NV12 test - verify size calculation fix
fn main() {
    ...
    println!(
        "  UV plane height: {} (rounded up from {})",
        uv_height, height
    );
    ...
    println!(
        "  UV plane height: {} (rounded up from {})",
        uv_height_odd, height_odd
    );
    println!(
        "  UV plane size: {} ({}x{})",
        uv_size_odd, width, uv_height_odd
    );
    ...
    println!(
        "\nNV12 size calculation for {}x{}:",
        small_width, small_height
    );
    println!(
        "  Y plane size: {} ({}x{})",
        y_small, small_width, small_height
    );
    println!(
        "  UV plane height: {} (rounded up from {})",
        uv_h_small, small_height
    );
    println!(
        "  UV plane size: {} ({}x{})",
        uv_small, small_width, uv_h_small
    );
    ...
    println!("\n  VERIFICATION:");
    if uv_offset + uv_small <= data.len() {
        println!(
            "  ✓ UV plane fits within buffer ({} + {} = {} <= {})",
            uv_offset,
            uv_small,
            uv_offset + uv_small,
            data.len()
    ...
    // Show what would have happened with old calculation
    let old_total = y_small + old_uv_size;
    println!("  Old total buffer size would have been: {}", old_total);
    if uv_offset + uv_small <= old_total {
        println!("  ✓ Would fit with old buffer");
    } else {
        println!("  ✗ Would OVERFLOW with old buffer (causes double-free/corruption)");
    }

    println!("\n=== NV12 Memory Corruption Fix Verified ===");
}
